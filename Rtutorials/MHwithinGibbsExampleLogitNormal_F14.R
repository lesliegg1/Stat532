...
log.phi.cc(phi.data[1], y.data[1], 0, 1)
 ...
 ## we end up with using rejection sampling.  For each run of the funtion
 ## it will continue to propose values until one is accepted.
  ...
  reject.draw.cc.phi <- function(y.i, mu, tau) {
   ...
   ### Check it?  ####
    test.out <- t(replicate(100, reject.draw.cc.phi(y.data[1], mu=0, tau=1)))
    ...
    prop.rejects <- sum(test.out[,1]-1)/sum(test.out[,1])  #rejects/total proposals
    prop.rejects
 ...
 ## 
    MH.draw.cc.phi <- function(phi.cur, y.i, mu, tau) {
     ...
     return(c(phi.cur,ind.jump, r))
    }

MH.draw.cc.phi(.8, 2, mu=-10, tau=1)
...
par(mfrow=c(5,2))
  for (j in 1:10){
    plot(1:n.gibbs, phi.mat[,j], type="l", main=expression(phi[j]), ylab=expression(phi[j]),  
...
burn.in <- 1:10
par(mfrow=c(1,2))
  hist(mu.vec[-burn.in], col=gray(0.85), nclass=100, freq=F, main="Posterior for mu", xlim=c(0,2))
    ...
    legend(0.5,4, legend=c("truth", "posterior mean", "empirical avg"), col=c("darkgreen",
                                2,4), lty=c(1,1,1), lwd=c(2,2,2), bty="n")
...
par(mfrow=c(2,5))
  for (j in 1:10){
    hist(phi.mat[-burn.in,j], main=expression(phi[j]), ylab=expression(phi[j]),  
...
my.caterpillar <- function(mat) {
  ...
  q5.out <- apply(mat, 2, function(x) quantile(x, c(0.025, 0.25, 0.5, 0.75, 0.975)))
  dev.new()
  plot(seq(min(q5.out[1,])-0.5,max(q5.out[5,])+0.5,length=n.p), 1:n.p, type="n", xlab=" ", ylab=" ",
        yaxt="n")
   segments(q5.out[1,], 1:n.p, q5.out[5,], 1:n.p, lwd=2, col=1)
   segments(q5.out[2,], 1:n.p, q5.out[4,], 1:n.p, lwd=4, col="purple")
   points(q5.out[3,], 1:n.p, pch="|", cex=1.5, col=2)
