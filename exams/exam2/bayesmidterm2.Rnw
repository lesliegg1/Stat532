...
\item When I first looked at this prior, I thought he was trying to give higher density to values of $\mu$ and $\pi$ that yield smaller numbers of successes. But then, on second thought, I thought maybe he is trying to specify a non-informative prior by spreading density out over a large range of values for $\pi$ and $\mu$. 

\item When I look at the plot of the Raftery Prior, it looks like small values of $\lambda$ with any value of $\pi$ are most probable. When I generated draws from the priors I specified in (i), however, it looks like values of $\lambda$ between $0$ and $500$ are equally likely, with values of $\pi$ close to $0$ or $1$ being most probable. These priors don't seem very similar at all. I included the code for this part in the appendix. 
