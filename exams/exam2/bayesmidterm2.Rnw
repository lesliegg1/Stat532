...
\item When I first looked at this prior, I thought he was trying to give higher density to values of $\mu$ and $\pi$ that yield smaller numbers of successes. But, on second thought, I think he was just trying to specify a vague prior over the entire range of possible expected observations. I do think he was trying to get at a non-informative prior. 
 
\item When I look at the plot of the Raftery Prior, it looks like small values of $\mu$ with any value of $\pi$ are most probable. When I generated draws from the priors I specified in (i), however, it looks like values of $\mu$ between $0$ and $500$ are equally likely, with $\pi$ either very close to $0$ or very close to $1$. It seems like the raftery prior makes lower expected number of observations most probable by making lower values of $\lambda$, and thus $m$, more probable. The prior used in $i$ makes values of $\lambda$ between $0$ and $500$ equally likely, and values of $\pi$ close to $0$ or $1$ are most likely across all values of $\lambda$. I included the code for this part in the appendix. 
...
\item The posterior distributions of $\alpha$ and $\beta$ are shown below. The regression lines displaying the fitted logits, and the fitted probability curves are shown for all $4000$ posterior draws are also shown.
...
hist(alpha, freq = F, xlab=expression(alpha), nclass=40, main=expression(alpha), col="grey", ylim = c(0, 0.11))
abline(v=coefs[1], col="red", lwd=3)
...
legend(-75, 0.07, bty="n", c("Cauchy Priors", "More Informative"), fill=c("lightgrey", "lightgreen"), cex = 0.8)

#beta plot

hist(beta, freq=F, xlab=expression(beta), nclass=40, main= expression(beta), 
     col="grey", ylim = c(0, 0.09))
abline(v=coefs[2], col="red", lwd=3)

hist(beta.inform, freq=F, nclass=30, main= expression(beta), add=TRUE, col = "lightgreen")

hist(beta.bad, freq=F, nclass=30, add=TRUE)
legend(20, 0.07, bty="n", c("Cauchy Priors", "More Informative"), fill=c("lightgrey", "lightgreen"), cex = 0.8)
@
...
<<stdprior, echo=FALSE, out.width=".5\\linewidth">>=
